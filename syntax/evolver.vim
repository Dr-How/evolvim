...
syn region sePreamble		start="\%^" end="^\ze\s*vertices" fold contains=TOP,@seElementAttr,@seElementsList,seProgram

syn region seParameter		matchgroup=seDefParameter start="^\s*PARAMETER" end="$" contains=seOptParameter
syn keyword seOptParameter	containedin=seParameter ON_ASSIGN_CALL

syn region seConstraint		matchgroup=seDefConstraint start="^\s*CONSTRAINT" end="$" contains=seOptConstraint
syn keyword seOptConstraint	containedin=seConstraint GLOBAL CONVEX NONNEGATIVE NONPOSITIVE NONWALL CONTENT_RANK
...
syn keyword seOptBoundary	containedin=seBoundary PARAMETERS CONVEX
...
syn keyword seOptQuantity	containedin=seQuantity ENERGY FIXED CONSERVED INFO_ONLY LAGRANGE_MULTIPLIER TOLERENCE MODULUS FUNCTION GLOBAL METHOD

syn region seMethod		matchgroup=seDefMethod start="^\s*METHOD_INSTANCE" end="$" contains=seOptMethod,seMethods
syn keyword seOptMethod		containedin=seMethod METHOD MODULUS ELEMENT_MODULUS GLOBAL
